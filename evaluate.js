...
function score_user(
	user_tweet,
	current_maximum_engagement_rate
	current_maximum_interaction_rate
	)
{
	...
	if ((candidate.followers_count>1000)&&(candidate.followers_count<10000)){
		...
		// Var initialization 
		var target = candidate.screen_name;
		var favourites = 0;
		var retweets = 0;
		var follow = 0;
		var posts_with_interactions = 0;
		var sensitive_content_posts = 0;
		var historical_tweets = '';

		// Async query for historical tweets
		T.get('statuses/user_timeline',{screen_name:target,count:200}).
			then(
				(response)=>{
		...
		content_interaction-rate = (posts_with_interactions)/historical_tweets.length;
		...
		if(user_engagement_rate>0.02){
			for (var i = historical_tweets.length - 1; i >= 0; i--) {
				T.get('statuses/retweets/:id',{id:historical_tweets[i].id}).
					then(
						(response)=>{
							sentiment_grade = 0;
		...
		grader =  new scorer();
		return [grader.network.activate([content_interaction-rate,user_engagement_rate,sensitive_content_rate,average_sentiment_grade]),historical_tweets];
		}
		else{
			return 'Unqualified';
		}
...
async function batch_async(tweet_list){
	...
	for (var i = if(results.length - 1; i >= 0; i--) {
